...
public class Terrain {
    ...
    private static final double SCALE = 50.0; // Adjust the scale to change the level of detail
    private static final double PERSISTENCE = 0.5; // Adjust the persistence to change the smoothness
    private static final int OCTAVES = 4; // Adjust the number of octaves to change the complexity
    
    public static Block generateFloor(int x, int y) {
        if (x > Vars.world.width()/2-11 && x < Vars.world.width()/2+11 &&
            y > Vars.world.height()/2-11 && y < Vars.world.height()/2+11) {
